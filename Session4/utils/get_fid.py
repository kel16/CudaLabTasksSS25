...
def create_samples(model, num_samples, device):
    z = torch.randn(num_samples, model.latent_dim).to(device)
    decoded_conv = model.decoder_fc(z)
    x_dec = decoded_conv.view(num_samples, 128, 8, 8)
    samples = model.decoder_deconv(x_dec)
    
    return samples

@torch.no_grad()
def get_fid(model, dataloader, device, num_samples = 1000):
    """" 
    ...
    samples = create_samples(model, num_samples, device)
