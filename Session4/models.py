...
class CondConvVAE(nn.Module):
    ...
    ConvTranspose layers for decoder, and Linear layers only for mu and sigma
    """
    def __init__(self, in_size=(3, 64, 64), latent_dim=32, n_classes=3, act="ReLU"):
        super().__init__()
        ...
        # Latent variables
        self.fc_mu = nn.Linear(self.flat_dim + self.n_classes, latent_dim)
        self.fc_sigma = nn.Linear(self.flat_dim + self.n_classes, latent_dim)

        # Decoder (fc -> unflatten -> ConvTranspose2d)
        self.decoder_fc = nn.Linear(latent_dim + self.n_classes, self.flat_dim)
    ...
    def forward(self, x, y):
        ...
        y = F.one_hot(y, self.n_classes).float().to(x.device)

        x_enc = self.encoder_conv(x)
        x_flat = x_enc.view(batch_size, -1)
        
        # Concatenate the output of encoder with label
        x_cat = torch.cat([x_flat, y], dim=1)

        mu = self.fc_mu(x_cat)
        log_var = self.fc_sigma(x_cat)
        z = self.reparameterize(mu, log_var)
        
        # Concatenate the input to decoder with label
        z_cat = torch.cat([z, y], dim=1)

        x_dec_flat = self.decoder_fc(z_cat)
        x_dec = x_dec_flat.view(batch_size, 128, x_enc.size(2), x_enc.size(3))
        x_hat = self.decoder_deconv(x_dec)
